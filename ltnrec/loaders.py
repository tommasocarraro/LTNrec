...
class TrainingDataLoaderLTNGenres2:
    """
    Data loader to load the training set of the MindReader dataset. It creates batches and wrap them inside LTN
    variables ready for the learning. This loaders differs from the TrainingDataLoaderLTNGenres. In particular,
    it randomly pick user-item pairs for which the user does not like a genre of the item from the entire set of these
    pairs, instead of focusing on the batch level.
    """
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            np.random.shuffle(idxlist)
            np.random.shuffle(self.u_i_dislike)
        ...
        for _, start_idx in enumerate(range(0, n, self.batch_size)):
            ...
            u_i_decrease = self.u_i_dislike[start_idx:end_idx]

            u_decrease = torch.tensor(u_i_decrease[:, 0])
            i_decrease = torch.tensor(u_i_decrease[:, 1])
            gt = torch.zeros_like(u_decrease)

            yield (ltn.Variable('users', torch.tensor(data[:, 0]), add_batch_dim=False),
                   ltn.Variable('items', torch.tensor(data[:, 1]), add_batch_dim=False),
                   ltn.Variable('ratings', torch.tensor(ratings), add_batch_dim=False)), \
                  (ltn.Variable('u_decrease', u_decrease, add_batch_dim=False),
                   ltn.Variable('i_decrease', i_decrease, add_batch_dim=False),
                   ltn.Variable('gt', gt, add_batch_dim=False))


class TrainingDataLoaderLTNGenres3:
    """
    Data loader to load the training set of the MindReader dataset. It creates batches and wrap them inside LTN
    variables ready for the learning. This loaders differs from the TrainingDataLoaderLTNGenres2. In particular,
    it creates LTN variables to reason on formulas which involve the genres of the movies that users like and dislike.
    """
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            np.random.shuffle(idxlist)
            np.random.shuffle(self.u_i_dislike)
            np.random.shuffle(self.u_i_like)
        ...
        for _, start_idx in enumerate(range(0, n, self.batch_size)):
            ...
            u_i_decrease = self.u_i_dislike[start_idx:end_idx]
            u_i_increase = self.u_i_like[start_idx:end_idx]

            u_decrease = torch.tensor(u_i_decrease[:, 0])
            i_decrease = torch.tensor(u_i_decrease[:, 1])
            gt_neg = torch.zeros_like(u_decrease)
            u_increase = torch.tensor(u_i_increase[:, 0])
            i_increase = torch.tensor(u_i_increase[:, 1])
            gt_pos = torch.ones_like(u_increase)
            ...
            yield (ltn.Variable('users', torch.tensor(data[:, 0]), add_batch_dim=False),
                  ...
                  (ltn.Variable('u_decrease', u_decrease, add_batch_dim=False),
                   ltn.Variable('i_decrease', i_decrease, add_batch_dim=False),
                   ltn.Variable('gt_neg', gt_neg, add_batch_dim=False)), \
                  (ltn.Variable('u_increase', u_increase, add_batch_dim=False),
                   ltn.Variable('i_increase', i_increase, add_batch_dim=False),
                   ltn.Variable('gt_pos', gt_pos, add_batch_dim=False))

from ltnrec.loaders import TrainingDataLoader, ValDataLoader, TrainingDataLoaderLTN, TrainingDataLoaderLTNGenres, \
    TrainingDataLoaderLTNGenres2, TrainingDataLoaderLTNGenres3
...
def grid_search(model_name, data, seed=None):
    """
    ...
    print("Starting grid search for %s model" % model_name)
                    ...
                    for mf_bias in config["mf_bias"]:
                        for alpha in config["alpha"]:
                            for p in config["p"]:
                                ...
                                train_loader = TrainingDataLoader(data.user_movie_ratings,
                                                                  b_size) if model_name == "mf" else \
                                    (TrainingDataLoaderLTN(data.user_movie_ratings, b_size) if model_name == "ltn" else
                                     (TrainingDataLoaderLTNGenres(data.user_movie_ratings,
                                                                  data.get_user_genre_ratings_dict(),
                                                                  data.movie_to_genres,
                                                                  b_size) if model_name == "ltn-genres" else
                                      (TrainingDataLoaderLTNGenres2(data.user_movie_ratings,
                                                                   data.get_user_items_genre(dislikes=True), b_size) if model_name == "ltn-genres-2" else
                                       TrainingDataLoaderLTNGenres3(data.user_movie_ratings,
                                                                    data.get_user_items_genre(dislikes=True),
                                                                    data.get_user_items_genre(dislikes=False),
                                                                    b_size))))

                                print("emb size %d - batch size %d - lr %.5f - l2 reg coeff %.5f - mf bias %d - alpha %s - p %s" %
                                ...
                                trainer = MFTrainer(model, optimizer) if model_name == "mf" else \
                                    (LTNTrainerMF(model, optimizer, alpha) if model_name == "ltn" else
                                     (LTNTrainerMFGenres2(model, optimizer, alpha) if model_name == "ltn-genres-3" else
                                ...
                                trainer.load_model("./saved_models/temp.pth")
    ...
    print("Best validation score for %s is hit@10 %.4f" % (model_name, best_val))
...
def experiment(models, data, proportions, test_metrics, starting_seed=0, n_runs=10):
    ...
    """
    results = {model: {"all_ratings": None,
                       "sparse_datasets": {seed: {} for seed in range(starting_seed, starting_seed + n_runs)}
                       }
               for model in models}
    val_loader = ValDataLoader(data.validation, 256)
    test_loader = ValDataLoader(data.test, 256)

    # for each seed, we perform the entire experiment
    for seed in range(starting_seed, starting_seed + n_runs):
        ...
        # for each model given in the procedure
        for model in models:
            print("Start experiment for %s model" % model)
            model_path = "./saved_models/%s.pth" % model
            config_path = model_path.replace(".pth", ".json")

            if not os.path.exists(model_path):
                # grid search if there is not a model checkpoint
                print("No model checkpoint found for %s, starting grid search" % model)
                grid_search(model, data, seed)

            # open best params file
            with open(config_path) as f:
                conf = json.load(f)

            if results[model]["all_ratings"] is None:
                # configure model
                ...
                trainer = MFTrainer(mf_model, optimizer) if model == "mf" else (
                    ...
                    (LTNTrainerMFGenres2(mf_model, optimizer, conf["alpha"]) if model == "ltn-genres-3" else
                     LTNTrainerMFGenres(mf_model, optimizer, conf["alpha"], conf["p"]))
                )

                # load model
                print("Load best model weights for %s" % model)
                trainer.load_model(model_path)

                # test model on entire dataset
                print("Test of %s trained on all the training ratings" % model)
                results[model]["all_ratings"] = trainer.test(test_loader, test_metrics)
            ...
            for p in p_dict:
                ...
                ratings = p_dict[p]
                train_loader = TrainingDataLoader(ratings, conf["batch_size"]) if model == "mf" else (
                    TrainingDataLoaderLTN(ratings, conf["batch_size"]) if model == "ltn" else
                    (TrainingDataLoaderLTNGenres(ratings,
                                                 data.get_user_genre_ratings_dict(),
                                                 data.movie_to_genres, conf["batch_size"]) if model == "ltn-genres" else
                     (TrainingDataLoaderLTNGenres2(ratings, data.get_user_items_genre(dislikes=True),
                                                  conf["batch_size"]) if model == "ltn-genres-2" else
                      TrainingDataLoaderLTNGenres3(ratings, data.get_user_items_genre(dislikes=True),
                                                   data.get_user_items_genre(dislikes=False), conf["batch_size"])))
                )

                if not os.path.exists("./saved_models/proportions/%s-%s_seed-%d.pth" % (model, p, seed)):
                    ...
                    print("No model checkpoint found for %s with seed %d on dataset with %s percent of training "
                          "ratings, starting training" % (model, seed, p))

                    print("Training %s on the dataset with %s percent of training ratings" % (model, p))
                    trainer.train(train_loader, val_loader, "hit@10", 200, 20, 1,
                                  "./saved_models/proportions/%s-%s_seed-%d.pth" % (model, p, seed))

                # load of the best model
                print("Loading best weights for %s trained on the dataset with %s percent of training ratings" % (
                model, p))
                trainer.load_model("./saved_models/proportions/%s-%s_seed-%d.pth" % (model, p, seed))

                # test of the model trained on the given proportion
                print("Test of %s trained on the dataset with %s percent of training ratings" % (model, p))
                results[model]["sparse_datasets"][seed][p] = trainer.test(test_loader, test_metrics)

    # save the dictionary as json in the results/ folder
    with open("./results/results3.json", 'w') as fp:
